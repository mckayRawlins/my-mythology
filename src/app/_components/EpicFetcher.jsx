...
export default function EpicFetcher() {
  ...
  const [ithaca, setIthaca] = useState(null);
  useEffect(() => {
    const fetchEpicData = async () => {
      try {
        ...
        const filteredData = epicData.filter((data) => data.id === "troy");
        setIthaca(filteredData);
        console.log(ithaca);
      } catch (error) {
        console.error(error);
  ...
  return (
    <div className="h-auto w-fit border mx-auto relative">
      <Link href="http://localhost:3000/epic/vengence">
        <div
          className="w-74 h-80 bg-yellow-500 mx-auto absolute top-0 left-0 flex justify-center items-center"
          style={{
